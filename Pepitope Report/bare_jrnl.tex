...
Peptides were encoded using the built in multilabel binarizer included in scikit learns preprocessing library.  Initially each amino acid is mapped to an integer.  The resulting lists of integers are then passed to the multilabel binarizer, producing a binarized list which can be passed to the SVM.
