...
\IEEEPARstart{T}{cell} epitopes are the part of an antigen, usually a specific peptide sequence, which is bound by the Major Histocompatibility Complex (MHC).  Two primary groupings of MHC proteins, MHC I and MHC II, are respectively present either on all cells or only antigen presenting cells.  Due to differences in the structure of their binding pockets, T~cell epitopes presented by MHC class I molecules are peptides between 8 and 11 amino acids in length, whereas those presented by MHC class II are 13-17 amino acids in length. 
